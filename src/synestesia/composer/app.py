#!/usr/bin/env python3
"""
Synesthesia Composer — MIDI-style piano-roll with node-based composition.
...
def draw_grid(surf, sf, comp, gr):
    ...
    for grp in groups.values():
        ...
        for i in range(1, len(grp)):
            ...
            d = max(1, int(math.hypot(x1 - x0, y1 - y0) / 4))
            for s in range(d):
                t0 = s / d; t1 = (s + 1) / d
                sx0 = x0 + (x1 - x0) * t0; sy0 = y0 + (y1 - y0) * t0
                sx1 = x0 + (x1 - x0) * t1; sy1 = y0 + (y1 - y0) * t1
                col = lerp_c(grp[i-1].color, grp[i].color, (t0 + t1) / 2)
                fade = max(50, 180 - int(abs(t0 - .5) * 200))
                fc = (col[0]*fade//255, col[1]*fade//255, col[2]*fade//255)
                pygame.draw.line(surf, fc, (int(sx0), int(sy0)), (int(sx1), int(sy1)), 2)
    ...
    for n in comp.notes:
        ...
        if comp.palette_pick and any(n.midi % 12 == LFI_DATA[p][0] for p in comp.palette_pick):
            hl_border = tuple(min(255, c + 80) for c in n.color)
            pygame.draw.circle(surf, hl_border, (int(nx), int(ny)), NR + 3, 2)
        elif n.hand == 'left':
...
def draw_keyboard(surf, sf, kb, active, comp):
    ...
    ly0 = ky - 14
    if ly0 > TB_H:
        leg = sf.render("Palette keys: 0..9 = G0..G9  - = G10  = = G11  C = clear", True, (60, 60, 75))
        surf.blit(leg, (4, ly0))
...
def main():
    ...
    pygame.display.set_caption("Synesthesia Composer")
    ...
    while running:
        ...
        if comp.playing:
            ...
            active = {comp.notes[i].midi for i in cur_idxs}
            new_idxs = [i for i in cur_idxs if i not in prev_sounding]
            prev_sounding = set(cur_idxs)
            for i in new_idxs:
